...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/admin/roles?${queryString}`, {
      headers: {
    ...
    console.error('Error proxying admin roles request:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error proxying admin role creation request:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
