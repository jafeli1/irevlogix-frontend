...
import { COMPLIANCE_CERTIFICATION_TYPES, BACKEND_URL } from '../../../../utils/constants';
...
export default function CertificationsTrackerDetail() {
  ...
  const [edited, setEdited] = useState<Partial<CertificationDetail>>({});
  ...
  const fetchDetail = async () => {
    try {
      const token = localStorage.getItem('token')!;
      const res = await fetch(`/api/compliance-tracker/certifications/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
        ...
        setError('Failed to load certification detail.');
      }
    } catch {
  ...
  const remove = async () => {
    ...
    if (res.ok) {
      router.push('/compliance-tracker/certifications-tracker');
    } else {
  ...
  if (!detail) {
    return (
      <AppLayout>
        <div className="text-center py-8">
          <h2 className="text-2xl font-bold text-gray-900">Certification not found</h2>
          <Link href="/compliance-tracker/certifications-tracker" className="text-blue-600 hover:text-blue-800 mt-4 inline-block">Back to Certifications Tracker</Link>
        </div>
      </AppLayout>
    );
  }
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Certifications Tracker Detail</h1>
              <p className="mt-1 text-sm text-gray-600">Edit compliance certification</p>
            </div>
            <div className="flex items-center space-x-3">
              <Link href="/compliance-tracker/certifications-tracker" className="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Back to List</Link>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        input.onchange = async (e: Event) => {
                          ...
                          fd.append('file', f);
                          const res = await fetch(`/api/compliance-tracker/certifications/${id}/upload`, {
                            method: 'POST',
                            headers: { Authorization: `Bearer ${token}` },
                            body: fd
                          });
                  ...
                  {hasPermission(permissions!, 'ProjectManagement', 'Delete') && (
                    <button
                      onClick={async () => {
                        ...
                        const token = localStorage.getItem('token')!;
                        const res = await fetch(`/api/compliance-tracker/certifications/${id}/file`, {
                          method: 'DELETE',
                          headers: { Authorization: `Bearer ${token}` }
                        });
