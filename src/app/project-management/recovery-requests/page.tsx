...
export default function RecoveryRequestsPage() {
  ...
  const fetchRecoveryRequests = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/recoveryrequests?${queryParams}`, {
        headers: {
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = 'recovery-requests.csv';
    a.click();
    ...
    return (
      <AppLayout>
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-600">You don&apos;t have permission to view recovery requests.</p>
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="mb-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            ...
            <Link
              href="/project-management/recovery-request-detail/new"
              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium"
            >
              Add New Recovery Request
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Filter Recovery Requests</h3>
              ...
              {recoveryRequests.map((request) => (
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Link
                      href={`/project-management/recovery-request-detail/${request.id}`}
