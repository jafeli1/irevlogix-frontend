...
export default function RecoveryRequestDetailPage() {
  ...
  const [formData, setFormData] = useState<RecoveryRequestData>({
  ...
  const fetchRecoveryRequest = async () => {
    try {
      ...
      const response = await fetch(`/api/recoveryrequests/${id}`, {
        headers: {
  ...
  const uploadFile = async (file: File, recoveryRequestId: number, documentType: string) => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('documentType', documentType);

    const token = localStorage.getItem('token');
    const response = await fetch(`/api/recoveryrequests/${recoveryRequestId}/upload`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isNew ? '/api/recoveryrequests' : `/api/recoveryrequests/${id}`;
      ...
      if (response.ok) {
        const recoveryRequestId = isNew ? (await response.json()).id : parseInt(id);
        ...
        for (const upload of fileUploads) {
          if (upload.file) {
            await uploadFile(upload.file, recoveryRequestId, upload.type);
          }
        }

        setSuccess(isNew ? 'Recovery request created successfully!' : 'Recovery request updated successfully!');
        
        if (isNew) {
          setTimeout(() => {
            router.push('/project-management/recovery-requests');
          }, 2000);
        }
      } else if (response.status === 401) {
        router.push('/login');
        ...
        setError(errorData || 'Error saving recovery request');
      }
    } catch (error) {
      console.error('Error saving recovery request:', error);
      setError('Error saving recovery request');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this recovery request?')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setSuccess('Recovery request deleted successfully!');
        setTimeout(() => {
          router.push('/project-management/recovery-requests');
        }, 2000);
      } else if (response.status === 401) {
        ...
        setError('Error deleting recovery request');
      }
    } catch (error) {
    ...
    return (
      <AppLayout>
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-600">You don&apos;t have permission to manage recovery requests.</p>
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">
          {isNew ? 'Add New Recovery Request' : 'Edit Recovery Request'}
        </h1>
        <p className="mt-2 text-gray-600">
          {isNew ? 'Create a new recovery logistics job request' : 'Update recovery request information'}
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe the recovery logistics request"
                />
              </div>
            </div>
          )}
                ...
                {[1, 2, 3, 4, 5].map((num) => (
                  <div key={num}>
                    ...
                    <input
                      ...
                      className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                    />
                    {formData[`assetsPhotoUpload${num}` as keyof RecoveryRequestData] && (
                      <p className="mt-1 text-sm text-gray-600">Current: {formData[`assetsPhotoUpload${num}` as keyof RecoveryRequestData]}</p>
                    )}
                  </div>
                ))}
              </div>
                ...
                {[1, 2, 3].map((num) => (
                  <div key={num}>
                    ...
                    <input
                      ...
                      className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                    />
                    {formData[`helpfulPhotoUpload${num}` as keyof RecoveryRequestData] && (
                      <p className="mt-1 text-sm text-gray-600">Current: {formData[`helpfulPhotoUpload${num}` as keyof RecoveryRequestData]}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="flex justify-between pt-6 border-t border-gray-200">
            ...
            <div className="flex space-x-4">
              <button
                type="button"
                onClick={() => router.push('/project-management/recovery-requests')}
              ...
              >
                {saving ? 'Saving...' : (isNew ? 'Save Recovery Request' : 'Update Recovery Request')}
              </button>
            </div>
          </div>
        </form>
